...
std::vector<int> w_lane_arr, y_lane_arr;
std::string group_name, video_path, calibration_path;
static int web_cam;
static double loop_rate_;
...
{
    nh.param<int>("/" + group_name + "/img_processing_node/web_cam", web_cam, 1);
    nh.param<double>("/"+group_name+"/img_processing_node/loop_rate_",loop_rate_,30);
    nh.param<std::string>("/" + group_name + "/img_processing_node/video_path", video_path, ros::package::getPath("img_processing") + "/src/vtest.avi");
    nh.param<std::string>("/" + group_name + "/img_processing_node/calibration_path", calibration_path, ros::package::getPath("img_processing") + "/src/camera_main.yaml");
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        {
            printf("File(%s) not found Please check the file name.\n", video_path);
            return 0;
        }
    }
    ...
    while (nh.ok())
    {
        ...
        SeulLaneDetection test_lane(frame, argv[1], y_lane_arr, w_lane_arr);
        cv::imshow(group_name + "test", frame);
